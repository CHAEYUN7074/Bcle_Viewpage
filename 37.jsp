...
if (searchKey != null) //-- 검색기능을 통해서 페이지가 요청되었을 경우
{
	// 넘어온 값이 GET 방식이라면...
...
{
	searchKey = "subject"; // 제목으로 검색기본표시
	searchValue = ""; // 검색칸은 비워두게
}

// 전체 데이터 갯수 구하기
int dataCount = dao.getDataCount(searchKey, searchValue); // 검색메소드에 따른 매개변수 변경!
...
// ex) 삭제되었을 때 → 나는 25페이지에 있는데 삭제해서 23페이지이면 23으로 마지막을 변경
...
// 실제 리스트 가져오기
List<BoardDTO> lists = dao.getLists(start, end, searchKey, searchValue); // 검색메소드에 따른 매개변수 변경!
